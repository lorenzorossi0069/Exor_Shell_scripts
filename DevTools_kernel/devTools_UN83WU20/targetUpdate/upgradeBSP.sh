...
if [[ $# < 1 ]] ; then
		echo "Error:"
		echo "arguments: < UN78 | UN65 | UN96 | UN60 | UN83 (ie WU20) >"
		exit 1
fi
...
if [[ $BSP_NAME == UN83 ]] ; then
	...
	#copyFileToTaget $SETUPDIR stm32_rproc.ko  /lib/modules/$(uname -r)/kernel/drivers/remoteproc  stm32_rproc.ko
	#copyFileToTaget $SETUPDIR remoteproc.ko   /lib/modules/$(uname -r)/kernel/drivers/remoteproc  remoteproc.ko
...
if [[ $BSP_NAME == UN83 ]] ; then
	...
	#copyFileToTaget  $SETUPDIR wifi_gpio            /etc/default wifi_gpio  #non serve: c'è già
